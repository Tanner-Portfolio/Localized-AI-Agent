...
## Project Overview
Project BMO is an autonomous, 100% offline, locally-hosted AI assistant. While themed around a fictional character, the primary engineering objective of this project is demonstrating secure, localized LLM inference without reliance on third-party cloud APIs. 

This project features a custom system-prompted Llama-3.2 personality, a natively cloned neural TTS voice, local computer vision, and an agentic logic loop running entirely on a Raspberry Pi 5.

## Architecture
* Compute: Raspberry Pi 5 (8GB RAM), Debian Trixie (Kernel 6.12).
* Brain (LLM): Llama-3.2-3B-Instruct, deployed via Ollama (.gguf Q4_K_M).
* Voice (TTS): Piper TTS ONNX model (Approx. 500-epochs).
...
## Persona Engineering & Prompt Optimization
* Model Optimization: Initially, parameter fine-tuning (QLoRA) was executed using canonical dialogue. However, local inference testing proved that a highly optimized system prompt on a 3B parameter model yielded significantly higher conversational coherence and persona accuracy on edge hardware, bypassing the output limitations of a highly restricted fine-tune.
* Voice Synthesis: Engineered a custom neural vocoder based on vocal performance. Through empirical testing, it was determined that a 500-epoch model retained the most natural vocal characteristics, as higher epochs (1,300+) introduced severe overfitting and audio degradation.
...
* Agentic Framework Integration: Upgrade from a reactive conversationalist to a ReAct logic agent, granting autonomous capabilities to check internal battery levels, manage files, and execute self-contained Python scripts.
